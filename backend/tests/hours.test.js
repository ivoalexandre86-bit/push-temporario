...
const { initTestDb, closeTestDb, getDb, getApp, login, withAuth, createProject, createArea, createAction, createUser, scopeUserToProject } = require('./helpers');
...
beforeAll(async () => {
  ...
  db = getDb();
  app = getApp();
  proj = await createProject(db, 'Projeto Horas');
  area = await createArea(db, 'Área Horas');
...
describe('Hours management', () => {
  ...
  it('actual hours are the sum of APPROVED actual time entries only', async () => {
    const action = await createAction(db, { projectId: proj, areaId: area, status: 'ANDAMENTO', plannedHours: 20 });
  ...
  it("a Contributor's actual-hours entries start PENDING and do not count until approved", async () => {
    const contributor = await createUser(db, { name: 'Colaborador Horas', email: 'contrib.horas@test.local', roleKey: 'CONTRIBUTOR' });
